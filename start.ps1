...
foreach ($fileName in $txtFiles.Keys) {
    Write-Host "$index: ${fileName}"  # Используем фигурные скобки для переменной
    $index++
}
...
if ($fileUrl) {
    ...
    # Запрашиваем у пользователя номер строки для скачивания
    $lineNumber = Read-Host "Введите номер строки для дальнейших действий"
