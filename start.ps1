...
$folderIndex = Read-Host "Введите номер папки для загрузки"

# Проверяем, что введённый номер корректен
if ($folderIndex -gt 0 -and $folderIndex -le $folders.Count) {
    $selectedFolderName = $folders[$folderIndex - 1] # Получаем название выбранной папки
    ...
    Write-Host "Неверный номер папки."
}
